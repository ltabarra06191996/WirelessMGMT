...
| NetAdapter 2    | __Bridged (Replicate)__ | 10.92.1.0 /24 |
...
### 10. Assign the DHCP server for the WLAN, `10.92.10.4`, finally `Apply to Device`.
...
### 13. Set the Name and Description to `AP-WLANs`. Then, under WLAN-POLICY Maps, `Add` a mapping and set the WLAN Profile to `Wireless-92` with a Policy Profile `Wireless-POL`. Then, `Apply to Device`.
...
### Exercise 03: Create WLANs for `THREAT-HUNTERS` on VLAN 12. Using the network address 10.92.12.0/24
